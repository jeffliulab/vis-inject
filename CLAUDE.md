...
### 2026-04-26 — GitHub repo 重命名 vis_inject → vis-inject

- `gh repo rename` 把 `jeffliulab/vis_inject` 改成 `jeffliulab/vis-inject`，并更新本地 `origin` remote URL（GitHub 自动保留旧 URL 的重定向）
- **故意未改**的 `vis_inject` 引用（HPC 物理目录名，与 GitHub repo 名解耦，按 rule #7 不动）：
  - `scripts/run_experiments.sh:14` — `PROJECT="/cluster/tufts/.../vis_inject"`
  - `scripts/hpc_pipeline.sh:10-22` — SBATCH `--output/--error` 路径 + `WORK_DIR`
  - `docs/HPC_GUIDE.md` — work dir / SLURM logs / `git clone ... vis_inject` / scp 路径（多处）
  - `CLAUDE.md:105`、`data/preparation/README.md:46`、`docs/experiment_report.md:211` — work dir 表格行
- 如果以后在 Tufts 上 `mv vis_inject vis-inject`，再统一替换上面这批
- HF 上的 `jeffliulab/visinject`（Space + Dataset）是独立 repo，与本次重命名无关
