...
def slide_7_dualdim():
    ...
    add_bullets(slide, MARGIN + Inches(0.3), bot_y + Inches(0.6),
                ...
                [
                    ...
                    "v1.3: DUAL-AXIS LLM judge (DeepSeek-V4-Pro thinking) + programmatic baseline. κ = 0.77 vs human (Claude Opus 4.7).  Cache-replay path lets reviewers re-derive paper numbers without an API key.",
                ], size=12)
...
def slide_14_results_headline():
    ...
    add_text(slide, sx, cy + Inches(0.85), card_w, Inches(1.0),
             "66.4 %", size=64, color=AMBER, bold=True, align=PP_ALIGN.CENTER)
    add_text(slide, sx + Inches(0.3), cy + Inches(1.95),
             card_w - Inches(0.6), Inches(0.4),
             "4 390 / 6 615 pairs (programmatic)",
    ...
    add_text(slide, rx2, cy + Inches(0.85), card_w, Inches(1.0),
             "0.756 %", size=64, color=RED, bold=True, align=PP_ALIGN.CENTER)
    add_text(slide, rx2 + Inches(0.3), cy + Inches(1.95),
             card_w - Inches(0.6), Inches(0.4),
             "50 / 6 615 pairs (broad)  •  2 verbatim",
             size=14, color=SUBINK, align=PP_ALIGN.CENTER)
...
def slide_15_per_vlm():
    ...
    bars = [
        ("Qwen2.5-VL-3B (3 B)",     8.94, NAVY,   "100 %"),
        ("Qwen2-VL-2B (2 B)",       8.90, ACCENT, "100 %"),
        ("DeepSeek-VL-1.3B (1.3 B)", 8.87, ACCENT, "98.6 %"),
    ...
    kpis = [
        ...
        ("Disruption",    "66.4 %",    "programmatic, all 6 615 pairs"),
        ("Inj. (broad)",  "0.756 %",   "50 / 6 615 — any non-none"),
        ("Inj. (verbatim)","2",        "URL on a code screenshot"),
    ]
    cy = chart_y
    ch = Inches(1.05)
